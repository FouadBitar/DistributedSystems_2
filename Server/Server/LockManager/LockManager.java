				...
				synchronized (this.lockTable) {
					...
					if (!bConflict) {
						...
						if (bConvert.get(0) == true) {
							...
							//remove the read lock from the table
							TransactionLockObject xLockObject_read_clone = (TransactionLockObject)xLockObject.clone();
							xLockObject_read_clone.setLockType(TransactionLockObject.LockType.LOCK_READ);
							DataLockObject dataLockObject_read_clone = (DataLockObject)dataLockObject.clone();
							dataLockObject_read_clone.setLockType(TransactionLockObject.LockType.LOCK_READ);
							this.lockTable.remove(xLockObject_read_clone);
							this.lockTable.remove(dataLockObject_read_clone);

							//add the write lock to the table
		...
		for (int i = 0; i < size; i++)
		{
			...
			{    
				...
				else if (dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_WRITE)
				{
					...
					// (2) transaction already had a WRITE lock, this lock request is redundant
