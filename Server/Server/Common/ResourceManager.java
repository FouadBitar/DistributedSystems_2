...
public class ResourceManager implements IResourceManager
{
	...
	public boolean addFlight(int xid, int flightNum, int flightSeats, int flightPrice) throws RemoteException
	{
		...
		// grab write lock from now
		try{
			if(lm.Lock(xid, "m_data", TransactionLockObject.LockType.LOCK_WRITE)) {
				//store data this method is going to write to in case of abort
				RMItem prevObj = readData(xid, Flight.getKey(flightNum));
				tm.writePreviousValue(xid, Flight.getKey(flightNum), prevObj);
			...
			//lock returned false - paramters are invalid 
			else {
				Trace.info("RM::addFlight(" + xid + ") could not add flight, invalid parameters");
				return false;
			}

		}
		//If there is a deadlock, do we wait for the abort from the client to remove it from the active transactions list
		//or do we remove it from the point of catching the deadlock. 
    ...
    public boolean commit(int transactionId) throws RemoteException, TransactionAbortedException, InvalidTransactionException {
		...
		if(tm.isActiveTransaction(transactionId)) {
			//delete the values of the data stored for restoring from the transaction manager
			tm.removePreviousValues(transactionId);
    ...
    public void abort(int transactionId) throws RemoteException, InvalidTransactionException {
		//check if it is an active transaction, otherwise it is an invalid one and throw exception
		...
		if(!isValid) {
			...
			for (Map.Entry<String, RMItem> entry : prev_data.entrySet()) {
				...
				//we must have the write locks for these keys 
				writeData(transactionId, key, value);
			}
			//then unlock the locks held
			lm.UnlockAll(transactionId);

			//remove the transaction from active and clear the previous writes data
			tm.removeActiveTransaction(transactionId);
			tm.removePreviousValues(transactionId);
		}
